...
// prototipos
void bubble_sort(int *p, int len, int (*fcomp)(int x, int y), void (*fswap)(int *m, int *n)); //void swap(int* p, int* q) esta era la funcion, ahora quiero un puntero a funcion: 
...
int main(void){
    ...
    print_array(a, SIZE);
    bubble_sort(a, SIZE, comp_ascendente, swap);    //le paso una funcion de swap, ahora yo elijo que quiero swappear
    print_array(a, SIZE);
...
void bubble_sort(int *p, int len, int (*fcomp)(int x, int y), void (*fswap)(int *m, int *n)){
    ...
    for(i=0; i<len-1; i++)
        for(j=i+1; j<len; j++)
	        if(fcomp(p[i], p[j]))  // llamo a la funcion de comparacion mediante el puntero
		        fswap(&p[i], &p[j]); // llamo a la funcion de swap mediante el puntero
}
