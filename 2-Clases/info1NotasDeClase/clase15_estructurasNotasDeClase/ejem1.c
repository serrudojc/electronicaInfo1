...
// defino el formato del tipo de dato, aun no guardo nada en memoria. La estructura tiene campos/miembros.
struct punto {	
                int x;
                int y;
             };
...
// ver apuntes clase sobre typedef
typedef struct {
                int x;
                int y;
               } spunto_t; 		//nuevo tipo de dato. s=structura, nombre, _t =type


// prototipos
void imprime_punto(spunto_t p); //funcion q recibe una estructura del tipo punto
spunto_t crea_punto(int x, int y); //recibo dos enteros, y devuelvo una estructura del tipo punto

// main
int main(void){
    struct punto p1;        // declaro una estructura sin inicializar, declaro una INSTANCIA de la estructura
    spunto_t p2 = {4, 23};  // declaro una estructura usando el tipo definido con typedef
                            // y la inicializo
    spunto_t p3;	//es una estructura del tipo punto, definida en algun typedef (revisar los .h)
